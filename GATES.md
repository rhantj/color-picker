...
- **진단에서 조합으로 가는 연결은 18개 중 2개뿐이다.** 축이 팔레트 관계어(`유사색` `보색`
  `톤 통일` `절제 안 함`)를 그대로 쓰는 진단만 이을 수 있다. 나머지 16개는 축이 `톤 (명도)`(33단계 전엔 `톤축 (명도)`)
...
**되돌릴 길은 상태를 안 늘리고 만든다.** LLM 이 고른 항목에 `(LLM 배정)` 을 붙이면(31단계부터 `(처음 값)` — 화면 문구를 뺐다),
되돌리기 버튼과 그 상태를 따로 두지 않고도 원래로 돌아갈 수 있다.

S21-G1 고르개가 재질 넷을 한글 이름으로 준다 · 처음 배정에만 `(처음 값)` 표시(31단계에서 `(LLM 배정)` 을 뒤집음) · 화면과 엔진의 재질 목록이 일치
...
- **실측이 요구한 단계가 아니다.** 대표가 미리 대응하기로 했다. 코퍼스가 실제로 자주 바뀌는 날이 오면
  TTL(2초)·지문(mtime+size) 방식을 다시 본다.
- **같은 초 안의 두 저장은 크기가 같으면 놓친다.** mtime 해상도가 낮은 파일시스템에서다. 편집기는 보통
  크기를 바꾸므로 실용상 문제없다 `[판단]`.
- **옛 벡터 창에 새로 추가된 문서는 임베딩 결과에 없다** — BM25 로만 잡힌다(수십 ms). 지워진 문서는
  걸러진다(`S27-G5`). **삭제만 하면 창이 없다** — 남은 문서가 전부 캐시에 있어 임베딩 호출 자체가
  없다. G5 가 처음에 삭제만 해서 헛돌았고, 문서 하나를 함께 고쳐 창을 열었다 `[실측]`.
- **G5 는 세 번 헛돌았다.** ① 지운 문서가 상위 3 에 우연히 안 들었고, ② 삭제만 하면 창이 없었고,
  ③ 스텁 지연이 질의 임베딩까지 늦춰 예산(0.7초)에 걸려 임베딩 없이 1단계로 답했다. 지금은 스텁이
  `zzqq` 질의에 지운 문서의 벡터를 돌려주고(코사인 1, 반드시 1위) 그 한 건만 지연에서 뺀다. 필터를
  빼면 서버가 500 으로 죽는 것을 확인했다 `[실측]`. **양성 대조 없이 통과한 게이트는 통과가 아니다.**
- **씨앗 풀·구조 카탈로그·재질은 여전히 기동 때 고정이다.** `/api/expand` 가 코퍼스 씨앗은 새 코퍼스에서
  찾지만 씨앗 풀은 옛것이다. 구조 카탈로그는 LLM 프롬프트가 이름을 그대로 써서 요청 중에 바뀌면 안 된다(기존 결정).
- **사다리 표시 3 은 이제 안 나온다.** 재적재는 늘 가능하므로 임베딩이 준비되면 곧 4 다.
- **기동 시점에 이미 깨진 코퍼스는 기동 실패다**(1단계 계약, exit 1). 4단계가 지키는 것은 "뜬 뒤에 깨지는 것" 이다.
- **스텁 벡터로는 1단계·1위를 단정할 수 없다.** 3차원 해시 벡터라 정확 매칭에 절대 동의하지 않고, 서로
  다른 문장이 코사인 0.997 로 붙기도 한다. 27단계 게이트는 "계속 답하는가 · 무엇이 안 나오는가 ·
  건수" 만 잰다. 순위 품질은 26단계가 실제 모델로 잰다.
- **저장 계층이 하나 늘자 옛 검사기 아홉이 저장소 `var/` 를 더럽혔다** (리뷰 지적). 임베딩 캐시가
  `var/embeddings.json` 에 남게 되면서, 데이터 폴더를 안 넘기던 검사기(2·3·13·14·15·17·25·26)가 서버를
  띄울 때마다 캐시를 썼다. 26단계 것은 스텁 벡터가 든 캐시를 남겼고 다음 자식이 그것을 읽어 `S26-G3`
  이 헛돌았다. 전부 서버마다 빈 임시 `TONEFIRST_DATA_DIR` 을 주도록 고쳤다. 임시 폴더는 OS 가 치운다.
- **처음 구현은 재임베딩 중에 코퍼스가 또 바뀌면 두 번째 문서 목록을 유실했다** (리뷰 재현). `prepare` 가
  진행 중인 것을 그대로 돌려줬고, 첫 번이 성공으로 끝나 `refresh` 의 재시도에도 안 걸렸다. 지금은 끝난 뒤
  마지막 문서 목록이 다르면 한 번 더 돈다. `S27-G2` 가 연속 편집 2건 → 임베딩 입력 2건으로 본다.
- **삭제만 하면 디스크 캐시가 안 줄던 결함이 있었다** (리뷰 재현). 새로 만든 것이 있을 때만 썼다. 가지치기로
  줄어도 쓴다. `S27-G6` 이 삭제 뒤 캐시 항목 33 을 본다.

## 28단계 — 어긋난 BM25 는 안 믿는다 · E1

26단계가 남긴 오답 2건("너무 병원 같아요" · "CTA 가 안 눌리게 생겼어요")은 결합(RRF)이 **BM25 오답에도
순위 점수를 줘서** 임베딩 1위(`dx-clinical` · `dx-weak-accent`)를 밀어낸 것이다. BM25 가 확신했는데 임베딩
1위와 다르다는 것은 그 확신이 흔한 어절 하나에서 왔다는 뜻이고, 그 순위는 증거가 아니다.
...
- **처음 처방은 어긋난 BM25 를 전부 버렸고 `S27-G1` 이 잡았다.** "zzqq 별똥별 카페" 처럼 희귀 어절 **둘**이
  통째로 맞은 확신까지 0 이 되어 정확 매칭이 무너졌다(스텁 임베딩이 동의하지 않으므로). E1 이 겨냥한 것은
  흔한 어절 **하나**로 우긴 확신이다. 그래서 어절 전체 매치가 `DISTRUST_MAX_WHOLE`(1) 이하일 때만 안 믿는다.
  좁힌 규칙에서도 실측 14/18 이다 `[실측]`. **한 게이트가 다른 단계의 게이트를 깨뜨리면 처방이 넓은 것이다.**

- **가중치 0 은 "어긋난 BM25 를 결합에서 뺀다" 는 뜻이다 — 후보로도 안 남는다.** BM25 에만 있고 임베딩
  순위에 없는 문서는 0점이라 결과에 못 든다. 코퍼스 34건은 전부 임베딩 순위에 있으므로 지금은 아무것도
  잃지 않는다. 임베딩이 못 잡은 문서가 BM25 로만 잡히는 경우가 생기면 0.5 같은 중간값을 다시 잰다 —
  스파이크에서 0.5 는 12/18 이라 지금은 값이 없다 `[실측]`.
- **처음 구현은 "임베딩 순위에 없음" 을 "어긋남" 과 같이 봤고 리뷰가 잡았다.** 27단계의 옛 벡터 창에서 방금
  더한 문서는 임베딩 순위에 아예 없다. 그때 어절 하나로 BM25 가 확신하면 가중치 0 이 되어 그 문서가 결과에서
  사라졌다. 지금은 순위에 **있으면서** 1위와 다를 때만 안 믿는다. `S28-G4` 가 새 문서 + 지연 재임베딩으로 본다 —
  이 게이트도 두 번 헛돌았다(고친 문서는 옛 벡터로 순위에 남는다 · "zzqq별똥별" 은 토크나이저가 둘로 쪼개
  어절 매치가 2개) `[실측]`.
- **남은 오답 2건은 이 처방 밖이다.** "사막에 홀로 핀 꽃"(임베딩도 못 잡음) · "빈티지 레코드 가게"(저신뢰로
  정직하게 "못 잡음"). 어휘 부재 2건은 E2(별칭 보강)의 몫이다.
- **음성 대조는 스파이크가 대신했다.** 가중치를 1 로 두면 정답 12 로 G1 이 운다 `[실측]`.

## 29단계 — 사용자 말투가 별칭에 없으면 임베딩도 못 잡는다 · E2

26단계 실측에서 "화면이 죽어 보여요" · "숨이 막혀요 화면이" 가 `dx-noisy` 로 갔다(어휘 부재). 진단 코퍼스의
별칭은 BM25 색인과 임베딩 문장(`embedText`) **둘 다**에 들어가므로, 별칭이 없으면 두 경로가 함께 못 잡는다.
...
- **별칭은 말투를 하나씩 좇는다.** "죽어 보인다" 를 넣어도 "죽은 느낌" 은 "죽은" 이 어절로 안 맞는다(별칭 "죽었다" 와
  다른 활용형). 활용형을 형태소로 푸는 대신 실제 오답에서 한 건씩 더하는 것이 이 코퍼스의 방식이다(4단계 결정).
  실측 오답이 쌓이면 그때 다시 잰다.
- **단독 일반어 별칭("막힌다" · "죽었다")은 반대 방향 위험이 있다** — "선택이 막힌다" 같은 무관한 질의가 어절 하나로
  BM25 에 걸릴 수 있다(리뷰 지적). 어절 하나짜리 확신은 28단계가 임베딩과 어긋나면 안 믿으므로 임베딩이 있을 때는
  결합이 거르고, 없을 때는 1단계 거짓 확신이 될 수 있다. 실측 사례가 없어 별칭은 두고 여기만 남긴다.
- **G1 은 BM25 만 본다.** Ollama 없이 돌리므로 임베딩이 동의하는지는 G3(실제 모델)가 본다 — G3 와 브라우저에서 "숨이 막혀요
  화면이" 는 1단계 확신, "화면이 죽어 보여요" 는 **3단계**(임베딩 1위가 달라 확신을 거두고 결합이 `dx-flat-value`
  를 골랐다 · 코사인 0.532)로 잡혔다 `[실측]`. 별칭이 임베딩 문장에도 들어가 결합이 맞는 쪽으로 기운 것이다.
- **구현 전 G1 에서 "숨이 막혀요 화면이" 는 `pair-02` 를 1단계로 거짓 확신했다** — 임베딩이 없으면 26단계의
  어긋남 판정도 없어서다. 별칭이 들어가면 진단의 어절 매치(2)가 팔레트(1)를 이긴다. 음성 대조는 이 구현 전 실행이다.

## 30단계 — 임베딩 2.2초 튐은 모델이 내려간 것이다 · E3

26단계부터 임베딩이 가끔 2.2초 걸렸다. 원인 `[실측]`: `/api/embed` 요청에 `keep_alive` 가 없어 Ollama 기본값
5분이 지나면 `bge-m3` 가 GPU 에서 내려가고, 다음 질의가 다시 올리는 값을 치른다(콜드 1855·2198ms · 웜 39~83ms).
...
- **30분이 지나면 같은 값을 한 번 치른다.** `keep_alive` 는 붙들어 두는 시간이지 영구가 아니다. 점심 뒤 첫 질의는
  재적재(약 2초)를 내고 확신 경로 예산에 걸려 3단계를 한 번 건너뛴다 — 26단계 이전과 같은 동작이다. 영구(`-1`)로
  두지 않은 이유는 GPU 8GB 를 다른 일과 나눠 쓰기 때문이고, 채팅 모델과 같은 30분에 맞췄다.
- **Ollama 는 올릴 때의 `keep_alive` 를 기억한다** `[실측]`. 올라와 있는 모델에 값 없는 요청을 보내도 만료가 짧아지지
  않는다. 그래서 G2 는 반드시 먼저 내린다 — 안 내리고 재면 구현 전에도 통과한다(실제로 그랬다).
- **`keep_alive: 0` 은 비동기다** `[실측]`. 바로 다음 요청이 옛 러너를 타서 콜드가 51ms 로 나왔다. G2·G3 는 `/api/ps`
  에서 사라질 때까지 기다린 뒤 잰다.
- **G3 는 처방이 아니라 원인을 잰다.** 콜드 ≥ 웜 3배가 아니면 이 기계에서는 재적재가 튐이 아니라는 뜻이라 실패한다.
  처방(요청에 값이 실리는가 · 만료가 남는가)은 G1·G2 가 본다.
- **한 번 관측된 8초 시간 초과는 따로 남긴다** `[실측]`. 채팅 모델(5.2GB)이 GPU 에 있는 상태에서 임베딩 모델을 다시
  올리다 저신뢰 예산 8초까지 넘긴 적이 한 번 있다. 재적재 자체가 원인이므로 같은 처방이 덮지만, 두 모델이 GPU 를
  다투는 조건에서는 재적재가 2초보다 훨씬 길 수 있다.

## 31단계 — 화면에서 검색 사다리와 "LLM 이 …" 문구를 뺀다 (대표 지시)

화면 아래의 검색 구조 사다리(1 전문검색 … 6 전체 사전임베딩)와, 결과 위의 단계 문장("1단계 · … LLM 을 부르지
않았습니다"), 맨 위 소개의 LLM 문장, 배색 구조·재질 카드의 "LLM 이 고름 / 배정" 배지를 뺀다. 재질 고르개의
...
S31-G2 서버가 내주는 홈에 사다리와 "LLM" 이 없고, 재질 고르개 표시가 `(처음 값)` 이며 다른 항목에는 안 붙는다
...
- **폴백을 화면에서 알 길이 없어졌다.** 14·17단계는 "LLM 이 골랐다" 배지가 거짓말하지 않는지를 지켰다. 배지를
  빼면서 그 보호도 화면에서는 사라졌다 — 서버 응답의 `selection.from` · `finishes.from` 은 그대로라 기록과 API 로는
  안다. 대표 지시의 결과이고, 14-G7 · 26-G7 은 "배지가 없다" 로 뒤집었다.
- **펼치기 핸들러는 게이트 밖이다.** 배지 블록을 지우다 `fin` 선언까지 지워 "펼치기" 가 `fin is not defined` 로
  죽었는데, S14·S17·S18·S21 게이트가 전부 통과했다 `[실측]` — 그 게이트들은 `structureCard` 를 DOM 스텁으로 직접
  부르지 `app.js` 의 클릭 핸들러를 안 탄다. 브라우저 실측(펼치기 → 카드 8장 · 고르개 표시)으로 잡았다. **문구를
  지우는 변경은 "없다" 만 재기 쉽고, "지운 자리 뒤가 아직 돈다" 는 실제 화면에서만 안다.**
- **빈 `expand__note` 가 남는다.** "펼치는 중…" 자리로 쓰다가 결과가 오면 비운다. 빈 `<p>` 라 여백만 남는다.

## 32단계 — 사이트 이름을 "톤먼저" 에서 "Color Picker" 로 (대표 지시)

워드마크 · 세 화면의 `<title>` · 서버 기동 문구 · 내보내기 파일의 머리말과 `generatedBy` · 내려받는 파일 이름
(`tonefirst-*` → `color-picker-*`)을 바꾼다. 환경변수(`TONEFIRST_DATA_DIR` 등)와 실행 설정 이름은 안 바꾼다 —
사용자에게 보이는 이름이 아니라 내부 식별자이고, 바꾸면 게이트 수십 개와 실행 환경이 함께 흔들린다.
라틴 워드마크(`TONEFIRST`)는 새 이름이 이미 라틴이라 뺀다.

S32-G1 화면·서버·내보내기 원본에 "톤먼저"·"TONEFIRST"·"tonefirst-" 파일 이름이 없고, 세 화면의 제목과 워드마크가 "Color Picker" 다 (정적 · 주석 제외)
    CHECK: node scripts/check-stage32.mjs S32-G1
    EXPECT: S32_G1_OK

S32-G2 서버가 내주는 세 화면에 "Color Picker" 가 있고 옛 이름이 없으며, 내보내기 4형식의 파일 이름이 `color-picker-*` 이고 본문의 출처가 "Color Picker" 다
    CHECK: node scripts/check-stage32.mjs S32-G2
    EXPECT: S32_G2_OK

S32-G3 화면·내보내기 회귀 — S2 8개 · S6 8개 · S20 7개 그대로 통과
    CHECK: node scripts/check-stage32.mjs S32-G3
    EXPECT: S32_G3_OK

### 알려진 한계 (32단계)

- **레이아웃은 게이트가 안 본다.** 워드마크가 3글자 두 칸에서 12글자 한 칸이 되어 상단바 줄바꿈 구간이 바뀔 수 있었다.
  브라우저 실측 `[실측]`: 워드마크 폭 136 → 125px, 636·700px 두 줄 · 701·768px 한 줄 · 375px 넘침 없음. 700 경계는 그대로다.
  `app.css` 의 상단바 주석에 수치를 다시 적었다(리뷰 지적).
- **안 바꾼 내부 식별자 목록**: 환경변수 `TONEFIRST_*` · 실행 설정 `tonefirst` · 게이트 임시 폴더 `tonefirst-gate-` ·
  `localStorage` 키 `tonefirst:mode`(바꾸면 저장된 모드가 초기화된다) · `design/` 의 초안 목업(서버가 내주지 않는 역사
  기록). `S32-G1` 의 옛 이름 정규식은 이것들을 일부러 안 잡는다.
- 워드마크 클래스는 `wordmark__ko` 였는데 영문이 들어가 `wordmark__name` 으로 바꿨다(리뷰 지적).

## 33단계 — "축" 이라는 말을 "원인" 으로 (대표 지시)

진단 화면과 진단표에서 "축"(의심할 축 · 톤축 · 색상축)을 쓰지 않는다. 화면 문장은 "의심할 원인 — 톤 (명도)" 처럼
"원인" 으로, 진단표의 "톤축 (명도)" 는 "톤 (명도)", "색상축과 톤축은 독립 변수다" 는 "색상과 톤은 독립 변수다" 로.
"수축"(색이 줄어드는 성질) · "축축하다" 는 다른 낱말이라 그대로 둔다. JSON 필드 이름 `axis` 와 CSS 클래스 `dx__axis`
는 내부 식별자라 그대로다.

S33-G1 화면 원본(세 화면 · app.js · ui.js · history.js · saved.js)과 진단표·팔레트 자료의 사용자 문장에 "축" 이 없고("수축"·"축축" 제외), ui.js 에 "의심할 원인" · app.js 진단 안내줄에 "원인을 의심할지" 가 있다 (정적 · 주석 제외 · 37단계부터 홈 소개 문장은 없다)
...
- **"축" 을 글자로 세면 다른 낱말이 걸린다.** "수축"·"축축하다" 가 코퍼스에 있다. 검사기는 앞이 "수"·"축" 이거나 뒤가
  "축" 인 자리를 뺀다 — 첫 실행에서 "축축하다" 의 둘째 글자가 걸려 고쳤다 `[실측]`. 앞으로 "축소" 같은 낱말이 코퍼스에
  들어오면 이 예외 목록에 더해야 한다.
- **진단 문장은 BM25 색인에 들어간다.** `axis`·`detail` 은 `indexText` 에 들어가고 `embedText`(증상·별칭·처방)에는 안 들어간다
...
- ~~**저장이 없다.**~~ **36단계가 닫았다** — 씨앗 id `hex-RRGGBB` 로 `/api/saved/derived` 를 타고 서버가 재계산한다.
- **구조 8가지를 전부 보인다 — LLM 이 다섯을 고르지 않는다.** 헥스에는 인상(질의)이 없어 고를 근거가 없다.
- **"흰색" 은 코퍼스에 없다** — 34단계 한계 그대로. 낱말 입력은 코퍼스 최근접으로 간다.

## 36단계 — 코드 및 색상 탭 저장 · 탭 전환 시 입력 초기화 (대표 지시)

35단계가 남긴 F1 을 닫는다. 구조 카드는 `/api/saved/derived` 에 씨앗 id `hex-RRGGBB` 를 보내고 **서버가 `structuresFor` 로 색을
다시 계산**한다(화면이 보낸 색은 안 믿음 · `store.js` 규칙 4). 짝 카드는 `/api/saved` 로, 씨앗 풀 쌍도 같은 경로에 풀 조회를
더해 받는다(해설이 없어 유형은 "모름"). `/saved`·엔진 내보내기는 파생과 같은 길. 탭이 바뀌면 검색창을 비운다.

S36-G1 헥스 씨앗 저장 왕복 — `seedId: "hex-E07A5F"` 로 저장하면 가짜 색을 무시하고 재계산(두 모드) · `seedLabel` 이 `#E07A5F` · 같은 키는 덮어쓰고 비율·메모 이어받음 · `hex-GGGGGG`·`#E07A5F`·없는 구조는 400 · `/api/saved` 목록 · unreal/unity 에 id `hex-E07A5F-<구조>-<모드>` 로 나옴 · CSS/JSON 은 건너뜀 · `savedFields` 배지·씨앗
    CHECK: node scripts/check-stage36.mjs S36-G1
    EXPECT: S36_G1_OK

S36-G2 짝 저장 — 씨앗 풀 쌍(`wada-002`)이 `/api/saved` 로 저장되고 이름은 두 원명, 2색에 원명, 비율 50:50 · 코퍼스 쌍(데이터의 첫 D형)은 전과 같이(70:30 · 유형·해설 유지) · 없는 id 400 · 같은 쌍 재저장은 덮어씀 · `savedFields` 가 유형 모름으로 그림
    CHECK: node scripts/check-stage36.mjs S36-G2
    EXPECT: S36_G2_OK

S36-G3 `/api/color` 가 재질 기본 배정을 준다 — 모든 구조의 모든 역할에 재질 id · 이름표 · LLM 아님(Ollama 없이 200 · `from` 이 llm 아님 · 응답에 model 키 없음)
...
S36-G5 회귀 — S9 6 · S18 13 · S19 6 · S20 7 · S22 6 · S35 6 = 44
    CHECK: node scripts/check-stage36.mjs S36-G5
    EXPECT: S36_G5_OK
