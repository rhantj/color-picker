...
S8-G3 빈 메모를 명시적으로 보내면 지운다 — 보존과 삭제를 구분한다 (S8-G2 의 음성 대조)
...
- **S8-G4 · S8-G7 은 `public/app.js` 를 정규식으로 보는 정적 검사다.** 회귀 스모크지 동작 증명이
  아니다. 실제 동작은 브라우저에서 따로 확인했다 — 입력 렌더·maxLength 200·재저장 시 메모 보존.
  S8-G4 는 특히 "note 라는 낱말이 호출 본문 근처에 있는가" 만 보므로, 전송 코드를 지우고 그 자리에
  note 가 든 주석만 남기면 통과한다. 알고 남긴 구멍이다.
...
- **양방향 텍스트 제어문자(U+202A~U+202E 등)는 내보내기에서 걸러지지 않는다.** CSS 파싱을 깨지는
  않지만 에디터 표시 순서를 뒤집을 수 있다(Trojan Source 계열). 내보낸 CSS 를 다른 프로젝트에
  붙여넣는 것이 이 모듈의 용도라 실재하는 경로다. **알고 미뤘다.**
