...
S21-G1 고르개가 재질 넷을 한글 이름으로 주고 LLM 배정에만 표시가 붙는다 (프로토타입 키 4가지 포함)
...
S21-G3 카드가 역할마다 고르개를 그리고, 지금 배정으로 열리고, 고치면 알린다 (읽기 전용 회귀 포함)
    CHECK: node scripts/check-stage21.mjs S21-G3
    EXPECT: S21_G3_OK

S21-G4 배정 상태가 redraw 밖에 있고 카드가 합쳐진 배정을 받는다 · 고칠 때 서버를 안 부르고 다시 그리지도 않는다
...
- **`S21-G4`·`S21-G5` 는 정적 검사라 표기만 본다.** 요청이나 배정을 다른 변수로 조립해
  호출부 밖에서 바꿔치기하면 우회한다. 실질 방어는 순수 함수 `finishOverrides` 쪽이고
  그것은 `S21-G2`·`S21-G6` 이 직접 부른다.

- **`S21-G4` 의 창을 한 번 잘못 잡았다.** `onFinish` 부터 300자를 봤더니 바로 뒤의
