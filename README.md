...
| 경로 | 무엇 |
|---|---|
| `/` | 탭 셋 — **색감 추천**: 자연어로 묻고 팔레트 또는 진단을 받는다. **캐릭터 색감**: 캐릭터 외형을 문장으로 쓰면 부위 여섯(피부·머리·눈·상의·하의·강조)의 색과 재질을 카드 한 장으로 준다 — 말한 색은 그대로, 나머지는 인상으로 고른 배색 쌍과 규칙, 헥스는 전부 코퍼스 80색. **코드 및 색상**: `#E07A5F` 같은 헥스나 "테라코타"·"빨강" 같은 색 이름을 넣으면 배색사전에서 그 색과 가장 가까운 색이 든 쌍 셋(짝)과, 그 색을 씨앗으로 한 배색 구조 여덟을 준다 — LLM 없이 결정적. 짝과 구조 모두 저장된다(36단계). 조합을 배색 구조로 펼치면 **로컬 LLM 이 여덟 중 다섯을 골라** 보여주고, 면적을 슬라이더로 맞춰 본다. 면적 비율 슬라이더, 메모와 함께 조합 저장. 진단의 축이 조합의 관계를 말하면 그 조합을 이어 보여준다 |
...
| 화면 | `public/` |
| 게이트 | `GATES.md` + `scripts/check-stage{1..36}.mjs` |
...
`GATES.md` 에 248개가 전부 있고 각 항목에 `CHECK:` / `EXPECT:` 가 붙어 있다.
...
| S20 | 7 | 파생을 엔진 수치로 · **두 엔진이 반전된 값** · 파일이 자기 엔진을 밝힘 · 발광 자리·EV 상한 · **손상 항목이 전체를 안 죽임** · 표기 토글 · 형식 조회 방어 |
| S21 | 7 | 재질 고르개 목록 · **바꾼 배정이 구조마다 따로** · 고른 값으로 열림 · **고칠 때 서버 0번·화면 안 부숨** · 저장에 실림 · 저장소 왕복 · 이름이 글자로만 |
...
| S26 | 7 | **거짓 확신 9건이 1단계에 안 남음(정답 7 → 12)** · 정확 매칭 회귀 · **임베딩 없어도 그대로** · 느린 임베딩에 안 끌림 · 결합·판정 결정적 · 오류 원문 경계 · 기록(usedLlm) |
...
| S28 | 4 | **어긋난 BM25 는 안 믿는다 — 실측 정답 12 → 14** · 결합 가중치 결정적 · 26단계 회귀 7개 · **옛 벡터 창의 새 문서는 "아직 모름"이라 BM25 로 살아 있음** |
| S29 | 3 | **사용자 말투가 별칭에 없으면 두 경로가 함께 못 잡음 — 별칭 8개로 실측 정답 14 → 16** · 별칭 사본·유일성·무결성 · 28단계 G1 조건 유지 |
| S30 | 4 | **임베딩 2.2초 튐 = 모델이 내려간 것 — 모든 임베딩 요청에 `keep_alive`** · 내린 뒤 서버 준비로 올리면 만료 25분 이상 · **콜드 재적재 ≥ 웜 3배(원인의 양성 대조)** · 26단계 회귀 7개 |
| S31 | 3 | **화면에 사다리·"LLM 이 …" 문구·단계 배지가 없다**(정적 · 서버 응답) · 고르개 표시 `(처음 값)` · 화면 게이트 5개가 뒤집힌 판정으로 통과 |
| S32 | 3 | **이름이 "Color Picker" — 세 화면 제목·워드마크·기동 문구·내보내기 머리말과 파일 이름** · 옛 이름 0 · 화면·내보내기 회귀 23개 |
| S33 | 3 | **"축" 이 화면·진단표·팔레트 문장에 없다("수축"·"축축" 제외) · "의심할 원인" 있음** · 서버 응답 · 코퍼스·진단 회귀 14개 |
| S34 | 10 | **캐릭터 외형 → 부위 여섯의 색 — 지어낸 색 0 · 말한 색 고정 · 종족이 피부를 정함 · 색상각 구획 하한(대부분 3)** · 파서 검증 · Ollama 없이 · 저장 왕복·엔진 6역할 · 탭 화면 · 회귀 65 |
| S35 | 6 | **헥스·색 이름 → 어울리는 색 — 입력 해석(헥스 4형·코퍼스 이름·낱말) · 80색 전부 자기 쌍이 3위 안(양성 대조) · 구조 8가지가 합성 씨앗 재계산과 일치(지어낸 색 0)** · `/api/color` Ollama 없이·400 셋 · 탭 셋 화면 · 회귀 29 |
