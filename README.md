...
| 경로 | 무엇 |
|---|---|
| `/` | 탭 셋 — **색감 추천**: 자연어로 묻고 팔레트 또는 진단을 받는다. **캐릭터 색감**: 캐릭터 외형을 문장으로 쓰면 부위 여섯(피부·머리·눈·상의·하의·강조)의 색과 재질을 카드 한 장으로 준다 — 말한 색은 그대로, 나머지는 인상으로 고른 배색 쌍과 규칙, 헥스는 전부 코퍼스 80색. **코드 및 색상**: `#E07A5F` 같은 헥스나 "테라코타"·"빨강" 같은 색 이름을 넣으면 배색사전에서 그 색과 가장 가까운 색이 든 쌍 셋(짝)과, 그 색을 씨앗으로 한 배색 구조 여덟을 준다 — LLM 없이 결정적, 저장은 아직 없다. 조합을 배색 구조로 펼치면 **로컬 LLM 이 여덟 중 다섯을 골라** 보여주고, 면적을 슬라이더로 맞춰 본다. 면적 비율 슬라이더, 메모와 함께 조합 저장. 진단의 축이 조합의 관계를 말하면 그 조합을 이어 보여준다 |
...
| 화면 | `public/` |
| 게이트 | `GATES.md` + `scripts/check-stage{1..35}.mjs` |
...
`GATES.md` 에 243개가 전부 있고 각 항목에 `CHECK:` / `EXPECT:` 가 붙어 있다.
