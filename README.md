...
| 화면 | `public/` |
| 게이트 | `GATES.md` + `scripts/check-stage{1..39}.mjs` |
...
`GATES.md` 에 258개가 전부 있고 각 항목에 `CHECK:` / `EXPECT:` 가 붙어 있다.
...
| S39 | 10 | **탭 셋을 지우고 채팅창 하나 — `/api/chat` 라우터 · 되묻기 1회 · 대화 10턴 상한 · LangSmith 트레이스(키 없음/있음)** · 로컬 트레이스 기록 · 탭 소멸(`tabStore`·`asTab`·`applyTab`·`tab=` 없음) · 바로잡기 · 회귀(S4·S22·S34·S35·S36) |
