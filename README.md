...
| 화면 | `public/` |
| 게이트 | `GATES.md` + `scripts/check-stage{1..15}.mjs` |
...
`GATES.md` 에 108개가 전부 있고 각 항목에 `CHECK:` / `EXPECT:` 가 붙어 있다.
...
| S15 | 9 | 명도 방향 거울 · **기본 모드 무변경 회귀** · 어두운 바탕 · 단계 간격 · 채도 상한(전역·역할) · 옵션 방어 · **거울이 안 닿아야 할 자리** · 카드 스와치 선택자 |
