...
export function finishLine(fields, nameFor = null) {
   ...
   *
   * **문구가 아는 것보다 많이 말하지 않게 한다.** `finishesAdjusted` 는 항목 전체에 하나뿐인
   * 참·거짓이다 — 역할 셋 중 **하나라도** 기본값과 다르면 참이 된다(`src/store.js`).
   * 그런데 처음 문구가 `직접 고름` 이었고, 줄 끝에 하나 붙으니 **셋 다 손으로 골랐다는
   * 뜻으로 읽혔다**(리뷰 지적). 실제로는 하나만 고른 경우가 흔하다.
   *
...
export function savedFields(entry) {
  ...
  if (e.kind === "derived") {
    ...
    for (const c of colors) {
       ...
       *
       * **원문 JSON 으로 만든 진짜 공격(`JSON.parse('{"__proto__":"metal"}')`)으로도 같다** —
       * 그때는 own 속성이 되므로 `hasOwn` 이 있든 없든 결과가 같고, 애초에 그 이름이 이
       * 구조의 역할 목록에 없어 반복문이 묻지도 않는다. 리뷰와 게이트 양쪽에서 확인했다.
